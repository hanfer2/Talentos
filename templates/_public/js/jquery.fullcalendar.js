...
var defaults = {
	...
	dayNames: ['Domingo','Lunes','Martes','Mierc&oacute;les','Jueves','Viernes','S&acute;bado'],
...
function Calendar(element, options, eventSources) {
	...
	function select(start, end, allDay) {
		currentView.select(start, end, allDay===undefined ? false : allDay);
	}
...
function AgendaEventRenderer() {
	...
	function slotSegHtml(event, seg, className) {
		var toolbar =  opt('editable')?"<div class='fc-event-toolbar'><span class='ui-icon ui-icon-close'></span><span class='clear'></span></div>":"";
		className += opt('editable')? " fc-event-editable":"";
		return "<div class='" + className + event.className.join(' ') + "' style='position:absolute;z-index:8;top:" + seg.top + "px;left:" + seg.left + "px'>" +
			"<a" + (event.url ? " href='" + htmlEscape(event.url) + "'" : '') + ">" +
				...
				"<span class='fc-event-time'>" + htmlEscape(formatDates(event.start, event.end, opt('timeFormat'))) + toolbar+"</span>" +
