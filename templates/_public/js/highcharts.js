	...
	tooltip: {
		...
		formatter: function() {
			var pThis = this,
				...
				xAxis = series.xAxis,
				x = pThis.x;
			return '<strong>'+ (pThis.point.name || series.name) +'</strong><br/>'+
				(defined(x) ? 
		...
		function setDOMEvents () {
			...
			container.onmousemove = function(e) {
				...
				if (mouseIsDown) { // make selection
					...
					if(typeof selectionMarker != 'undefined'){
						if (zoomHor) {
...
(function($){
	$.fn.vhighchart = function(settings){
		var series = []
		var headers = $("thead th:gt(0)", this)
		var xCategories = this.getColumn(0, ":not(.no-chart)")
		var table = this
		headers.each(function(i){
			var data = table.getColumn(i+1, ":not(.no-chart)")
			data = data.map(function(e){v=parseFloat(e); return isNaN(v)? 0: v})
			data = $.grep(data,isNumber)
			series.push({'name':$(".column-title",this).text(), 'data': data})
		})
		settings = $.extend({'series': series, 'title':{'text':this.attr('title')},
			'xAxis':{'categories':xCategories}, 'chart':{'renderTo':'chart-container'}},settings)
		$(this).addClass("table-chart")
		this.chart = new Highcharts.Chart(settings)
		return this;
	}
	$.fn.highchart = function(settings){
		var series = []
		var headers = this.getColumn(0, ":not(.no-chart)")
		var xCategories = []
		$("thead th:gt(0)", this).each(function(){xCategories.push($(".column-title",this).text())})
		var table = this
		$("tbody tr:not(.no-chart)", this).each(function(i){
			var data = []
			$("td:gt(0)",this).each(function(){
				v = parseFloat(this.innerText)
	...
	$.fn.piechart = function(settings){
		...
		var data = []
		$("tbody tr:not(.no-chart)", this).each(function(i){
			data.push([$("td:first", this).text(), parseFloat($("td:eq(1)", this).text())])
		})
		series.push({'name': name, 'data': data, type: 'pie'})
		settings = $.extend(true,{'series': series, 'title':{'text':this.attr('title')}, 
			'plotOptions':{'pie':
				{'allowPointSelect': true, cursor:'pointer',
					'dataLabels':{'enabled':true, 'color':'white'}
				}},
			'chart':{'renderTo':'chart-container'}},settings)
		$(this).addClass("table-chart")
		this.chart = new Highcharts.Chart(settings)
		return this;
	}
